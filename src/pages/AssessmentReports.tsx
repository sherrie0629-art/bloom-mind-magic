...
const AssessmentReports = () => {
  ...
  const getDesc = (r: Report) => {
    const d = r.result_data as any;
    return d.description?.slice(0, 60) + (d.description?.length > 60 ? "…" : "") || "";
  };
          ...
          reports.map((r, i) => {
            ...
            return (
              <motion.div
                ...
                onClick={() => navigate(`/assessment-reports/${r.id}`)}
                className="rounded-2xl bg-card p-4 shadow-card cursor-pointer active:scale-[0.98] transition-transform"
              >
                <div className="flex items-start gap-3">
                  <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${cfg.gradient}`}>
                    <Icon className="h-5 w-5 text-primary-foreground" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="font-display text-sm font-semibold text-foreground truncate">{getTitle(r)}</h3>
                    </div>
