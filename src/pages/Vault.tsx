...
const Vault = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const { data } = await supabase.from("story_vault").select("*").eq("user_id", user.id).order("unlocked_at", { ascending: false });
    ...
    <div className="min-h-screen bg-gradient-calm pb-20 md:pb-8">
      ...
      <div className="px-4 space-y-3">
        <AnimatePresence mode="wait">
          {loading ? <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-16 text-center text-sm text-muted-foreground">{t("vault.loading")}</motion.div>
