...
const SoulMap = () => {
  ...
  useEffect(() => {
    if (!user) { promptLogin(t("auth.promptViewSoulMap")); navigate("/"); return; }
    (supabase as any).from("soul_fragments").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).then(({ data }: any) => { if (data) setFragments(data); });
  ...
  return (
    ...
    <div className="min-h-screen pb-12" style={{ background: "linear-gradient(180deg, hsl(225 50% 8%), hsl(260 40% 12%), hsl(225 45% 10%))" }}>
      <SEO title={`${t("soulMap.title")} — ${t("home.appName")}`} description={t("home.tagline")} />
      <div className="flex items-center gap-3 px-5 pt-12 pb-4">
        ...
        <span className="ml-auto text-[11px] text-white/40">{t("soulMap.starsConst", { stars: fragments.length, c: unlocked.length })}</span>
      </div>
      {groupedFragments.map((group) => (
        <div key={group.key} className="px-5 mt-6">
          <h3 className="text-xs font-semibold text-white/50 mb-3 flex items-center gap-1.5"><span>{group.key === "assessment" ? "📐" : "💬"}</span>{t(`soulMap.groups.${group.key}`)}<span className="text-[10px] text-white/30 ml-1">({group.items.length})</span></h3>
          {group.items.length === 0 ? (<div className="rounded-2xl border border-white/10 p-5 text-center"><p className="text-xs text-white/30">{group.key === "assessment" ? t("soulMap.emptyAssessment") : t("soulMap.emptyChat")}</p></div>) : (
