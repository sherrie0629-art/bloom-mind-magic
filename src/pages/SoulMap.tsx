...
const SOURCE_GROUPS = [
  { key: "assessment", label: "测评之星", sources: ["mbti", "bazi", "zodiac", "emotion"] },
  { key: "chat", label: "对话之星", sources: ["chat"] },
];
...
const SoulMap = () => {
  ...
  const groupedFragments = SOURCE_GROUPS.map((g) => ({
    ...g,
    items: fragments.filter((f) => g.sources.includes(f.source_type)),
  }));
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-5 pt-12 pb-4">
        ...
        <h1 className="font-display text-lg font-semibold text-white/90">灵魂星图</h1>
        <span className="ml-auto text-[11px] text-white/40">
          {fragments.length} 颗星 · {unlocked.length} 个星座
        </span>
      </div>
      ...
      {groupedFragments.map((group) => (
        <div key={group.key} className="px-5 mt-6">
          <h3 className="text-xs font-semibold text-white/50 mb-3 flex items-center gap-1.5">
            <span>{group.key === "assessment" ? "📐" : "💬"}</span>
            {group.label}
            <span className="text-[10px] text-white/30 ml-1">({group.items.length})</span>
          </h3>
          {group.items.length === 0 ? (
            <div className="rounded-2xl border border-white/10 p-5 text-center">
              <p className="text-xs text-white/30">
                {group.key === "assessment" ? "完成测评获得星星 ✨" : "与角色深度对话获得星星 💫"}
              </p>
            </div>
          ) : (
              ...
              {group.items.map((f, i) => (
                <motion.button
                  ...
                  style={{
                    ...
                    boxShadow: `0 0 20px -4px ${f.color}60`,
      ...
      <div className="px-5 mt-8">
        <h3 className="text-xs font-semibold text-white/50 mb-3 flex items-center gap-1.5">
          <span>⭐</span>
          星座图鉴
          <span className="text-[10px] text-white/30 ml-1">({unlocked.length}/{ACHIEVEMENTS.length})</span>
        </h3>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {unlocked.map((ach, i) => (
            <motion.button
              ...
              onClick={() => setSelectedConstellation(ach)}
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[10px] text-white/40">已点亮</span>
              </div>
              <p className="text-xs font-semibold text-white/90">{ach.constellation.name}</p>
              <p className="text-[10px] text-white/50 mt-0.5 line-clamp-1">{ach.name}</p>
            </motion.button>
          ))}

          {/* Locked constellations */}
          {locked.map((ach, i) => (
            <motion.button
              key={ach.id}
              initial={{ opacity: 0, y: 10 }}
              ...
              onClick={() => setSelectedConstellation(ach)}
              className="rounded-2xl p-4 text-left border border-dashed border-white/10"
              style={{ background: "hsl(225 40% 15% / 0.5)" }}
            >
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-xl opacity-30">🔒</span>
              </div>
              <p className="text-xs font-medium text-white/30">{ach.constellation.name}</p>
              <p className="text-[10px] text-white/20 mt-0.5 line-clamp-1">???</p>
            </motion.button>
          ))}
        </div>
      </div>
      ...
      {locked.length > 0 && (
        <div className="px-5 mt-8">
          <div
            ...
            style={{ background: "linear-gradient(135deg, hsl(260 35% 20% / 0.6), hsl(225 50% 18% / 0.4))", border: "1px solid hsl(260 35% 65% / 0.15)" }}
          >
            <h4 className="text-xs font-semibold text-white/60 mb-2">🧭 下一个目标</h4>
            {locked.slice(0, 2).map((ach) => (
              <div key={ach.id} className="flex items-center gap-2.5 py-1.5">
                <span className="text-lg">{ach.icon}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-white/80 font-medium">{ach.constellation.name}</p>
                  <p className="text-[10px] text-white/40 truncate">{ach.description}</p>
                </div>
                {ach.agentId && (
                  <button
                    onClick={() => navigate(`/chat?agent=${ach.agentId}`)}
            ...
            <>
              <DialogHeader className="items-center text-center">
                <div
                  ...
                  style={{ backgroundColor: selectedFragment.color + "20" }}
                >
              ...
              <div className="flex items-center justify-between text-[11px] text-muted-foreground pt-2 border-t border-border">
                <span>
                  来源：{sourceLabels[selectedFragment.source_id || selectedFragment.source_type] || selectedFragment.source_type}
                </span>
                <span>
                  {new Date(selectedFragment.created_at).toLocaleDateString("zh-CN")}
                </span>
              </div>
            </>
          )}
              ...
              <DialogHeader className="items-center text-center">
                <div className="mx-auto mb-2 flex h-16 w-16 items-center justify-center rounded-full bg-muted">
                  <span className="text-3xl">
                    {unlockedIds.includes(selectedConstellation.id)
                ...
                <DialogDescription className="text-sm leading-relaxed">
                  {unlockedIds.includes(selectedConstellation.id)
                    ...
                    : `达成条件：${selectedConstellation.description}`}
                </DialogDescription>
              </DialogHeader>
              {!unlockedIds.includes(selectedConstellation.id) && selectedConstellation.agentId && (
                <div className="pt-2 text-center">
                  <button
                    onClick={() => {
                      setSelectedConstellation(null);
                      navigate(`/chat?agent=${selectedConstellation.agentId}`);
                    }}
