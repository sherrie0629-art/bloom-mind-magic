...
const MOODS = [
  ...
  { emoji: "🙂", label: "不错", score: 4 },
  { emoji: "😄", label: "很棒", score: 5 },
];

const QUICK_WORDS = ["疲惫", "焦虑", "平静", "开心", "兴奋", "孤独", "感恩", "迷茫"];
...
const DailyWhisper = () => {
  ...
  const handleGenerate = async () => {
    if (selectedMood === null && !inputText.trim() && !imagePreview) {
      toast.error("请选择情绪或输入内容");
      return;
    }
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("daily-whisper", {
        body: {
          inputText: inputText.trim() || moodWord || null,
          inputImageBase64: imagePreview || null,
          moodEmoji: mood?.emoji || null,
          moodWord: moodWord || mood?.label || null,
      ...
      // Show whisper text immediately; image will be null initially
      ...
      // Start polling for image if not yet available
      if (!data.imageUrl && data.whisperId) {
        startImagePolling(data.whisperId);
      }
  ...
  const startImagePolling = (whisperId: string) => {
    ...
    imagePollingRef.current = setInterval(async () => {
      ...
      try {
        const { data } = await supabase.functions.invoke("daily-whisper", {
          body: { action: "check-image", whisper_id: whisperId },
        });
        if (data?.imageUrl) {
  ...
  const handleGenerateReport = async () => {
    if (thisMonthCount < 3) {
      toast.error("本月至少签到3次才能生成月报");
      return;
    }
    setIsReportLoading(true);
    setMonthlyReport(""); // Show container immediately
    try {
      const session = (await supabase.auth.getSession()).data.session;
      if (!session) throw new Error("未登录");

      const resp = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/daily-whisper`,
        {
          method: "POST",
          headers: {
      ...
      if (!resp.ok) {
        ...
        toast.error(errData.error || "生成月报失败");
    ...
    try {
      ...
      if (result.imageUrl) {
        try {
          ...
          console.error("Image load error for save:", e);
        }
      }

      // Fallback: draw a decorative gradient card if no image
      if (!hasImage) {
        const cardW = 500, cardH = 300;
        const x = (canvas.width - cardW) / 2;
        const cardGrad = ctx.createLinearGradient(x, yPos, x + cardW, yPos + cardH);
        cardGrad.addColorStop(0, "#4a2c8a");
        cardGrad.addColorStop(0.5, "#6b3fa0");
        cardGrad.addColorStop(1, "#3d6098");
        ctx.save();
        roundedClip(ctx, x, yPos, cardW, cardH, 24);
      ...
      lines.forEach((line, i) => { ctx.fillText(line, canvas.width / 2, yPos + i * 50); });
      yPos += lines.length * 50 + 40;

      ctx.fillStyle = "rgba(255,255,255,0.4)";
      ctx.font = "22px 'Noto Sans SC', sans-serif";
      ctx.fillText("心灵密语 · 每日心语", canvas.width / 2, yPos);
      ...
      a.href = url;
      a.download = `心灵密语-${new Date().toLocaleDateString("zh-CN")}.png`;
  ...
  const handleShareAction = async () => {
    if (!result) return;
    if (navigator.share) {
      try { await navigator.share({ text: `"${result.whisper}" —— 心灵密语` }); } catch { /* cancelled */ }
    } else {
      await navigator.clipboard.writeText(`"${result.whisper}" —— 心灵密语`);
  ...
  const ChartTooltipContent = ({ active, payload }: any) => {
    ...
    if (!d.record) return <div className="rounded-lg bg-card p-2 text-xs text-muted-foreground shadow-card">无记录</div>;
    return (
  ...
  return (
    <div className="min-h-screen bg-gradient-calm pb-24">
      ...
      <div className="sticky top-0 z-10 flex items-center gap-3 bg-background/80 backdrop-blur-md px-4 py-3">
        ...
        <h1 className="font-display text-lg font-semibold text-foreground">每日情绪签到</h1>
      </div>
      ...
      <AnimatePresence mode="wait">
        {isGenerating ? (
          <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center px-6 py-20">
            ...
            <p className="mt-6 text-sm text-muted-foreground">正在为你创作专属密语…</p>
          </motion.div>
        ) : result ? (
          <motion.div key="result" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="px-6 py-6">
            <div className="relative overflow-hidden rounded-3xl shadow-soft">
              {result.imageUrl ? (
                <img src={result.imageUrl} alt="心语插画" className="h-80 w-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; (e.target as HTMLImageElement).nextElementSibling?.classList.remove('hidden'); }} />
              ) : null}
              <div className={`relative h-80 w-full bg-gradient-mystic ${result.imageUrl ? 'hidden' : ''}`}>
                {/* Show shimmer loading hint when image is being generated */}
                {!result.imageUrl && result.whisperId && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
                    ...
                    />
                    <p className="text-xs text-white/50">插画生成中…</p>
                  </div>
                )}
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                ...
                <p className="mt-3 text-xs text-white/50">
                  {new Date().toLocaleDateString("zh-CN", { month: "long", day: "numeric" })} · 心灵密语
                </p>
              </div>
            </div>
          ...
          <motion.div key="input" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="px-6 py-6 space-y-5">
            {/* ===== Zone 1: Mood Check-in ===== */}
            <div className="rounded-2xl bg-card p-5 shadow-card">
              <h2 className="mb-1 font-display text-sm font-semibold text-foreground">此刻，你的心情是？</h2>
              <p className="mb-4 text-[11px] text-muted-foreground">选一个表情，记录今天的情绪</p>
              ...
              <div className="flex justify-between px-2">
                {MOODS.map((m, i) => (
                  ...
                  >
                    <motion.span
                      animate={selectedMood === i ? { scale: 1.35 } : { scale: 1 }}
                      className={`text-3xl transition-opacity ${selectedMood !== null && selectedMood !== i ? "opacity-40" : ""}`}
                    >
                      {m.emoji}
                    </motion.span>
                    <span className={`text-[10px] ${selectedMood === i ? "font-semibold text-foreground" : "text-muted-foreground"}`}>
                      {m.label}
                    </span>
                  </button>
                ))}
              ...
              {/* Expand after selecting mood */}
              <AnimatePresence>
                {selectedMood !== null && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <div className="mt-4 pt-4 border-t border-border/30">
                      <p className="mb-2 text-[11px] text-muted-foreground">用一个词描述你的感受</p>
                      <div className="flex flex-wrap gap-2 mb-3">
                        {QUICK_WORDS.map((w) => (
                          <button
                            key={w}
                            onClick={() => setMoodWord(w)}
                            className={`rounded-full px-3 py-1 text-xs transition-colors ${moodWord === w ? "bg-primary text-primary-foreground" : "bg-muted/50 text-muted-foreground border border-border/40"}`}
                          >
                            {w}
                          </button>
                        ))}
                      ...
                      <input
                        ...
                        placeholder="或自己输入…"
                        maxLength={10}
                        className="w-full rounded-xl border border-border/50 bg-muted/30 px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/20"
                      />
                    </div>
                  </motion.div>
                )}
            ...
            {/* Advanced input: text + image */}
            <div className="rounded-2xl bg-card p-4 shadow-card">
              <p className="mb-3 text-[11px] text-muted-foreground">💡 想说更多？写一句话或上传图片，AI 会生成更精准的密语</p>
              <div className="flex items-start gap-3">
                <div className="mt-1 shrink-0 rounded-lg bg-secondary/10 p-1.5">
                ...
                <div className="flex-1 relative">
                  <textarea
                    ...
                    placeholder="今天的天气很好，心情也不错…"
                    maxLength={200}
                    className="w-full resize-none rounded-xl border border-border/50 bg-muted/30 px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                    rows={2}
                  />
              ...
              <div className="mt-3">
                {imagePreview ? (
                  <div className="relative inline-block">
                    <img src={imagePreview} alt="预览" className="h-20 w-20 rounded-xl object-cover shadow-card" />
                    <button onClick={() => { setImagePreview(null); if (fileInputRef.current) fileInputRef.current.value = ""; }} className="absolute -right-2 -top-2 rounded-full bg-destructive p-1 shadow-sm">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 rounded-xl border border-dashed border-border/60 bg-muted/20 px-3 py-2.5 text-xs text-muted-foreground transition-colors active:bg-muted/40">
                    ...
                    上传图片
                    <ImagePlus className="ml-auto h-3.5 w-3.5 text-muted-foreground/30" />
                  </button>
                )}
            ...
            <motion.button
              ...
              disabled={isGenerating || (selectedMood === null && !inputText.trim() && !imagePreview)}
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-mystic py-3.5 text-base font-semibold text-primary-foreground shadow-soft disabled:opacity-40 transition-opacity"
            >
      ...
      {/* ===== Zone 2: 14-Day Emotion Curve ===== */}
      {!isGenerating && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="px-6 mt-2">
          <div className="rounded-2xl bg-card p-4 shadow-card">
            <h3 className="mb-1 font-display text-sm font-semibold text-foreground">📈 14天情绪曲线</h3>
            <p className="mb-3 text-[10px] text-muted-foreground">点击节点回看当天的情绪记录</p>
            <div className="h-40">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} onClick={(e: any) => {
                  if (e?.activePayload?.[0]?.payload?.record) {
                    setSelectedNode(e.activePayload[0].payload.record);
                  }
                }}>
                    ...
                    dot={(props: any) => {
                      ...
                      return (
                        <circle
                          ...
                          cy={cy}
                          r={5}
                          fill={payload.record ? "hsl(var(--secondary))" : "transparent"}
                          stroke="hsl(var(--secondary))"
                          strokeWidth={2}
                  ...
                  <div className="mt-3 pt-3 border-t border-border/30 flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-foreground">
                        {selectedNode.mood_word || selectedNode.input_text?.slice(0, 20) || "无关键词"}
                        <span className="ml-2 text-muted-foreground font-normal">
                          {new Date(selectedNode.created_at).toLocaleDateString("zh-CN", { month: "short", day: "numeric" })}
                        </span>
                      </p>
                      <p className="mt-1 text-[11px] text-muted-foreground line-clamp-2">"{selectedNode.whisper}"</p>
                    </div>
                    <button onClick={() => setSelectedNode(null)} className="text-muted-foreground/50">
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                </motion.div>
      ...
      {/* ===== Zone 3: Monthly Report ===== */}
      {!isGenerating && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="px-6 mt-4">
          <div className="rounded-2xl bg-card p-5 shadow-card">
            <div className="flex items-center gap-3 mb-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-golden">
                <BookOpen className="h-5 w-5 text-primary-foreground" />
              </div>
              <div>
                <h3 className="font-display text-sm font-semibold text-foreground">情绪月报</h3>
                <p className="text-[10px] text-muted-foreground">
                  本月签到 {thisMonthCount} 次 {thisMonthCount < 3 ? "· 再签到几次即可生成" : "· 可以生成解读啦"}
                </p>
              </div>
            </div>
            ...
            {monthlyReport !== null ? (
                ...
                {!isReportLoading && monthlyReport && (
                  <div className="mt-3 flex gap-2">
                    <button
                      onClick={async () => {
                        if (navigator.share) {
                          await navigator.share({ text: monthlyReport });
                        } else {
                          await navigator.clipboard.writeText(monthlyReport);
                          toast.success("已复制到剪贴板");
                        }
                      }}
              ...
              <button
                ...
                disabled={isReportLoading || thisMonthCount < 3}
                className="w-full rounded-xl bg-gradient-golden py-3 text-sm font-semibold text-white shadow-soft disabled:opacity-40 transition-opacity"
              >
                {isReportLoading ? "暖暖正在为你写信…" : "生成本月情绪解读"}
              </button>
            )}
          </div>
        </motion.div>
      ...
      {!isGenerating && history.length > 0 && (
        <div className="px-6 mt-4 pb-4">
          <h3 className="mb-3 font-display text-sm font-semibold text-foreground flex items-center gap-2">
            📜 历史记录
            <span className="text-[10px] font-normal text-muted-foreground">共 {history.length} 条</span>
          </h3>
          <div className="space-y-2.5">
            {history.slice(0, 10).map((record, i) => (
              ...
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-muted/40 text-xl">
                  {record.mood_emoji || "✨"}
                </div>
