...
interface PurchaseRecord { id: string; product_type: string; product_id: string | null; amount: number; status: string; created_at: string; }

const PRODUCT_LABELS: Record<string, string> = { deep_report: "Deep Report", compatibility: "Chemistry Test" };
...
const Profile = () => {
  ...
  const premiumBenefits = [
    { label: "Daily Chats", free: "20", premium: "1000" },
    { label: "Daily Quizzes", free: "5", premium: "30" },
    { label: "AI Memory", free: "Last 5", premium: "Unlimited" },
    { label: "Deep Reports", free: "$1.99/ea", premium: "2/month" },
    { label: "Chemistry", free: "$1.99/ea", premium: "1/month" },
  ];
      ...
      <div className="px-6 pt-14 text-center">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mx-auto h-20 w-20 rounded-full bg-gradient-mystic p-0.5">
          <div className="flex h-full w-full items-center justify-center rounded-full bg-card">
            <span className="font-display text-2xl text-foreground">{subLoading ? "🌙" : plan === "premium" ? "👑" : "🌙"}</span>
          </div>
        </motion.div>
        ...
        <>
          {!subLoading && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
              className={`mt-6 mx-6 rounded-2xl shadow-card p-4 transition-colors duration-300 ${plan === "premium" ? "bg-gradient-to-br from-secondary/10 to-gold/10 border border-secondary/20" : "bg-card"}`}>
              <div className="flex items-center gap-2 mb-3">
                <Crown className={`h-5 w-5 ${plan === "premium" ? "text-yellow-500" : "text-muted-foreground"}`} />
                <span className="text-sm font-semibold text-foreground">{plan === "premium" ? "✨ Premium" : "Free Plan"}</span>
                {plan === "premium" && expiresAt && <span className="text-[10px] text-muted-foreground ml-auto">Expires: {new Date(expiresAt).toLocaleDateString("en-US")}</span>}
              </div>
              <div className="space-y-2.5 mb-3">
                <div>
                  <div className="flex justify-between text-[11px] text-muted-foreground mb-1"><span>Today's Chats</span><span>{chatCount}/{chatLimit}</span></div>
                  <div className="h-1.5 rounded-full bg-muted overflow-hidden"><div className="h-full rounded-full bg-gradient-golden transition-all duration-500" style={{ width: `${Math.min(100, (chatCount / chatLimit) * 100)}%` }} /></div>
                </div>
                <div>
                  <div className="flex justify-between text-[11px] text-muted-foreground mb-1"><span>Today's Quizzes</span><span>{assessmentCount}/{assessmentLimit}</span></div>
                  <div className="h-1.5 rounded-full bg-muted overflow-hidden"><div className="h-full rounded-full bg-gradient-mystic transition-all duration-500" style={{ width: `${Math.min(100, (assessmentCount / assessmentLimit) * 100)}%` }} /></div>
                </div>
              </div>
              {plan !== "premium" && (
                <button onClick={() => {}} className="w-full rounded-xl bg-gradient-golden py-2.5 text-xs font-semibold text-primary-foreground flex items-center justify-center gap-1.5">
                  <Sparkles className="h-3.5 w-3.5" /> Upgrade to Premium · $4.99/mo
                </button>
              )}
            </motion.div>
          )}
          ...
          {!subLoading && plan !== "premium" && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mt-3 mx-6 rounded-2xl bg-card shadow-card p-4">
              <h4 className="text-xs font-semibold text-foreground mb-2.5 flex items-center gap-1.5"><Crown className="h-3.5 w-3.5 text-secondary" /> Benefits Comparison</h4>
              <div className="space-y-0">
                <div className="grid grid-cols-3 text-[10px] text-muted-foreground pb-1.5 border-b border-border"><span>Benefit</span><span className="text-center">Free</span><span className="text-center text-secondary font-semibold">Premium</span></div>
                {premiumBenefits.map((b) => (
                  <div key={b.label} className="grid grid-cols-3 text-[11px] py-1.5 border-b border-border/50 last:border-0">
                    <span className="text-foreground">{b.label}</span><span className="text-center text-muted-foreground">{b.free}</span><span className="text-center text-secondary font-medium">{b.premium}</span>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          {purchases.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mt-4 px-6">
              <div className="rounded-2xl bg-card shadow-card p-4">
                <h4 className="text-xs font-semibold text-foreground mb-3 flex items-center gap-1.5"><ShoppingBag className="h-3.5 w-3.5 text-muted-foreground" /> Purchase History</h4>
                <div className="space-y-2">
                  {purchases.map((p) => (
                    <div key={p.id} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                      <div><p className="text-xs font-medium text-foreground">{PRODUCT_LABELS[p.product_type] || p.product_type}</p><p className="text-[10px] text-muted-foreground">{formatDate(p.created_at)}</p></div>
                      <div className="text-right">
                        <p className="text-xs font-semibold text-foreground">${(p.amount / 100).toFixed(2)}</p>
                        <p className={`text-[10px] ${p.status === "completed" ? "text-green-500" : "text-muted-foreground"}`}>{p.status === "completed" ? "Completed" : p.status === "pending" ? "Pending" : p.status}</p>
                      </div>
                    </div>
                  ))}
                </div>
