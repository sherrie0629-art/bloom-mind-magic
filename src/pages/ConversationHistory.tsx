  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-calm pb-24 md:pb-8">
      ...
      <div className="mt-6 px-6 space-y-3">
        ...
        : conversations.length === 0 ? (<div className="text-center py-16"><MessageCircle className="mx-auto h-12 w-12 text-muted-foreground/30" /><p className="mt-3 text-sm text-muted-foreground">No conversations yet</p><button onClick={() => navigate("/")} className="mt-4 rounded-xl bg-gradient-golden px-6 py-2 text-sm font-medium text-primary-foreground">Start Your First Chat</button></div>)
        : conversations.map((conv, i) => { const agent = agents.find((a) => a.id === conv.agent_id); return (
          <motion.button key={conv.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} onClick={() => navigate(`/chat?agent=${conv.agent_id}`)} className="flex w-full items-center gap-3 rounded-2xl bg-card p-4 shadow-card text-left">
            ...
            <div className="flex-1 min-w-0"><h3 className="text-sm font-semibold text-foreground truncate">{conv.title || `Chat with ${agent?.name || "AI"}`}</h3><p className="text-[11px] text-muted-foreground">{new Date(conv.updated_at).toLocaleDateString("en-US")}</p></div>
