      ...
      <div className="px-4">
        <div className="rounded-2xl bg-card p-4 shadow-card">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right"><p className="text-sm font-semibold text-secondary">{BOND_LABELS[currentLevel - 1]}</p><p className="text-[10px] text-muted-foreground">{totalTurns} conversations</p></div>
          </div>
          {nextThreshold && (<div className="mt-3">
            <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1"><span>{BOND_LABELS[currentLevel - 1]}</span><span>Next: {BOND_LABELS[currentLevel]} ({nextThreshold} turns)</span></div>
          ...
          {!nextThreshold && currentLevel >= 5 && (<p className="mt-3 text-[11px] text-secondary flex items-center gap-1"><Sparkles className="h-3 w-3" /> Max bond level reached</p>)}
        </div>
      </div>
      <div className="mt-6 px-4">
        <h3 className="mb-3 font-display text-sm font-semibold text-foreground flex items-center gap-2">📖 Story Fragments <span className="text-[10px] font-normal text-muted-foreground">Unlocked {currentLevel}/5</span></h3>
            ...
            <motion.div key={loreEntry.level} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }} className={`rounded-2xl border p-4 transition-all ${isUnlocked ? "border-secondary/20 bg-card shadow-card" : "border-border bg-muted/30"}`}>
              <div className="flex items-center gap-2 mb-2">
                <div className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${isUnlocked ? "bg-secondary/10 text-secondary" : "bg-muted text-muted-foreground"}`}>
                  {isUnlocked ? <Unlock className="h-2.5 w-2.5" /> : <Lock className="h-2.5 w-2.5" />} Lv.{loreEntry.level} · {BOND_LABELS[loreEntry.level - 1]}
                </div>
                {!isUnlocked && <span className="text-[10px] text-muted-foreground">Requires {BOND_THRESHOLDS[loreEntry.level - 1]} turns</span>}
              </div>
              {isUnlocked ? <p className="text-sm leading-relaxed text-card-foreground italic">"{loreEntry.text}"</p> : <p className="text-sm text-muted-foreground/50">??? Keep chatting with {agent.name} to unlock ???</p>}
            </motion.div>);
          })}
        </div>
      </div>
      <div className="mt-6 px-4 pb-6">
        <h3 className="mb-3 font-display text-sm font-semibold text-foreground flex items-center gap-2">🔮 Hidden Easter Eggs <span className="text-[10px] font-normal text-muted-foreground">Found {eggsFound.length}/{agent.easterEggs.length}</span></h3>
        <div className="space-y-3">
          {agent.easterEggs.map((egg, index) => { const isFound = eggsFound.includes(egg.trigger); return (
            <motion.div key={index} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + index * 0.1 }} className={`rounded-2xl border p-4 ${isFound ? "border-secondary/20 bg-card shadow-card" : "border-border bg-muted/30"}`}>
              {isFound ? (<><div className="flex items-center gap-2 mb-2"><span className="rounded-full bg-secondary/10 px-2 py-0.5 text-[10px] font-medium text-secondary flex items-center gap-1"><Sparkles className="h-2.5 w-2.5" /> Unlocked</span><span className="text-[10px] text-muted-foreground">Trigger: "{egg.trigger}"</span></div><p className="text-[11px] leading-relaxed text-muted-foreground line-clamp-3 italic">{egg.response.replace("【🔮 隐藏记忆解锁】", "").trim().slice(0, 120)}…</p></>) : (
                <div className="flex items-center gap-3"><div className="shrink-0 rounded-xl bg-muted p-2"><Lock className="h-4 w-4 text-muted-foreground/40" /></div><div><p className="text-sm font-medium text-muted-foreground/50">Undiscovered secret</p><p className="text-[10px] text-muted-foreground/40 mt-0.5">Try exploring certain topics in conversation…</p></div></div>
              )}
            </motion.div>);
          })}
        </div>
