...
const AssessmentFlow = () => {
  ...
  const fetchParallelUniverse = useCallback(async (mbtiType: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("assessment", { body: { action: "parallel-universe", mbtiType } });
  ...
  const fetchResult = async (finalHistory: QA[]) => {
    setLoading(true); setLoadingMsg("Analyzing your personality type...");
    try {
      const { data, error } = await supabase.functions.invoke("assessment", { body: { history: finalHistory } });
    ...
    } catch (e: any) { toast.error(e.message || "Failed to load, please retry"); } finally { setLoading(false); }
  };

  const handleStart = async () => {
    if (!user) { toast.error("Please sign in first 🌙"); navigate("/auth"); return; }
    if (!canAssess) { toast.error(`Daily quiz limit reached (${assessmentLimit}) 💫`); return; }
    await incrementAssessment(); setStarted(true); setLoading(true); setLoadingMsg("AI is preparing your questions...");
    try {
      const { data, error } = await supabase.functions.invoke("assessment", { body: { action: "batch-questions" } });
      if (error) throw error;
  ...
  const handleSharePoster = () => {
    ...
    sharePoster({
      ...
      bars: [
        { label1: "Extroverted E", label2: "Introverted I", value: result.traits.E_I },
        { label1: "Sensing S", label2: "Intuition N", value: result.traits.S_N },
        { label1: "Thinking T", label2: "Feeling F", value: result.traits.T_F },
        { label1: "Judging J", label2: "Perceiving P", value: result.traits.J_P },
      ],
      extraLines: parallelData ? [`🧙 Fantasy World: ${parallelData.magic.role}`, `🤖 Cyberpunk: ${parallelData.cyberpunk.role}`] : undefined,
    ...
    return (
      <div className="min-h-screen bg-gradient-calm flex flex-col">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <h2 className="text-sm font-semibold text-foreground">MBTI Personality Quiz</h2>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-calm pb-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="px-6">
          ...
          <div className="rounded-2xl bg-card p-5 shadow-card mb-4"><h3 className="font-display text-sm font-semibold text-foreground mb-3">Personality Analysis</h3><p className="text-sm text-muted-foreground leading-relaxed">{result.description}</p></div>
          <div className="rounded-2xl bg-card p-5 shadow-card mb-4">
            <h3 className="font-display text-sm font-semibold text-foreground mb-4">Dimension Analysis</h3>
            {[
              { l1: "Extroverted E", l2: "Introverted I", v: result.traits.E_I },
              { l1: "Sensing S", l2: "Intuition N", v: result.traits.S_N },
              { l1: "Thinking T", l2: "Feeling F", v: result.traits.T_F },
              { l1: "Judging J", l2: "Perceiving P", v: result.traits.J_P },
            ].map(b => (
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-2xl bg-card p-5 shadow-card mb-4">
            <h3 className="font-display text-sm font-semibold text-foreground mb-3 flex items-center gap-1.5"><Sparkles className="h-4 w-4 text-accent" /> Parallel Universe You</h3>
            {parallelLoading ? (<div className="space-y-3"><Skeleton className="h-16 rounded-xl" /><Skeleton className="h-16 rounded-xl" /></div>) : parallelData ? (
              <div className="space-y-3">
                <div className="rounded-xl bg-muted/50 p-3"><p className="text-xs font-semibold text-foreground mb-1">🧙 Fantasy World · {parallelData.magic.role}</p><p className="text-xs text-muted-foreground leading-relaxed">{parallelData.magic.description}</p></div>
                <div className="rounded-xl bg-muted/50 p-3"><p className="text-xs font-semibold text-foreground mb-1">🤖 Cyberpunk · {parallelData.cyberpunk.role}</p><p className="text-xs text-muted-foreground leading-relaxed">{parallelData.cyberpunk.description}</p></div>
              </div>
            ) : null}
          </motion.div>
          <div className="flex gap-3">
            <button onClick={handleSharePoster} className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-card py-3 text-sm font-medium text-foreground shadow-card"><Download className="h-4 w-4" /> Save Poster</button>
            <button onClick={() => navigate(`/chat?agent=barista`, { state: { mbtiResult: { mbtiType: result.mbtiType, title: result.title, description: result.description, parallelUniverse: parallelData || undefined } } })}
              className="flex-1 rounded-xl bg-gradient-golden py-3 text-sm font-semibold text-primary-foreground">Talk About It</button>
          </div>
        </motion.div>
